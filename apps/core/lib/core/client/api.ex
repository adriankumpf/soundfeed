defmodule Core.Client.API do
  ...
  defmacro __using__(_params) do
    quote do
      ...
      defp parse({:ok, %Response{status_code: 200, body: data}}),
        do: Poison.decode(data, as: body())

      defp parse({:ok, %Response{status_code: 401}}), do: {:error, :forbidden}
      defp parse({:ok, %Response{status_code: _, body: err}}), do: {:error, err}
      defp parse({:error, %Error{reason: reason}}), do: {:error, reason}

      @spec paginate(resp, tracks) :: {:error, any} | {:ok, User.t() | tracks}
      defp paginate({:error, reason}, _acc), do: {:error, reason}
      defp paginate({:ok, %User{} = user}, _acc), do: {:ok, user}
      defp paginate({:ok, %Page{collection: col, next_href: nil}}, acc), do: {:ok, acc ++ col}
      defp paginate({:ok, %Page{collection: col, next_href: url}}, acc), do: get(url, acc ++ col)
    end
  end
end
